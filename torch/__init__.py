...
if sys.platform == 'win32':
    ...
    if cuda_version is not None:
        print("DEBUG: cuda_version = ", cuda_version)
    else:
        print("DEBUG: no cuda version")

    for p in dll_paths:
        if glob.glob(os.path.join(p, 'cudart64*.dll')):
            print("DEBUG: glob.glob(os.path.join(p, 'cudart64*.dll')) : ", p)
    ...
    # Kineto (profiler) dependency on CUPTI
    if cuda_version:
        cuda_version_1 = cuda_version.replace('.', '_')
        cuda_path_var = 'CUDA_PATH_V' + cuda_version_1
        default_path = os.path.join(pfiles_path, 'NVIDIA GPU Computing Toolkit', 'CUDA', 'v' + cuda_version)
    ...
    print("DEBUG: torch/__init__.py: dll_paths = ", str(dll_paths), ", cuda_path = ", cuda_path, ", cuda_extras_path = ", cuda_extras_path)
